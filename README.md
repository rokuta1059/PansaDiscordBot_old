...
**메인 프로그램** 파일입니다. 토큰을 각자에게 주어진 값으로 수정하여 실행하게 됩니다.

#### 함수

- **def makeDiary()**
망언집이 저장된 파일을 읽어들이고 이를 리스트에 저장해줍니다. 리스트는 전역변수로 지정되어 사용됩니다.

- **def cypersRank(nickname)**
넥슨에서 서비스중인 게임 **사이퍼즈**의 닉네임명을 인자로 받아와서 검색한 다음 
닉네임, 클랜, 랭크, 공식전 RP, 최고 RP, 승패를 크롤링하여 리스트로 반환합니다.
...
특정 명령어를 받게 되는 경우 수행할 작동들을 구현합니다.
